...
class PagesController extends Controller
{
 public function __construct() {
    ...
    {   
        ...
        $pages = new Page();
        //its blog
        if (  !empty(Input::get('image-path'))  ) {
            $pages->image_option = 1;
            $pages->image_src = Input::get('image-path');

        } else {
            $pages->image_option = 0;
        }
    ...
    public function getPage($param1 = null, $param2 = null)
    {
        ...
        if (!isset($param2) && isset($param1)) { //LINK
            ...
            if (isset($page)) {
                $layout = "pages.website_pages.customize_page";
                if ($page->image_option == 1) {
                    $layout = "pages.website_pages.customize_blog";
...
public function postUploadSlider()
{
    ...
    public function postBlogUpload()
    {
        $status = 400;
        $img = $_FILES['file']['tmp_name'];
        $name = Job::generateRandomString(10). $_FILES['file']['name'];
        if (move_uploaded_file($img, 'assets/images/blog-images/tmp/' . $name)) {
            $status = 200;
        }
          
         return Response::json(array(
            'status' => $status,
            'path' => '/assets/images/blog-images/tmp/' . $name
        ));  
