    ...
    static public function PreparepagesForIndex($data) {
        ...
        if (isset($data)) {
            foreach ($data as $dkey => $dvalue) {
                ...
                if ($dvalue['image_option']==1) {
                    $extra = '<span class="label label-primary">Blog Post</span>';
                } else {

                    $extra = '<span class="label label-default">Page</span>';
                }
                if(isset($dvalue['status'])) {
                    switch ($dvalue['status']) {
                        case 1: // Set but not paid
                            $dvalue['status_message']= '<span class="label label-success">Public</span>&nbsp;'.$extra;
                            break;
                        case 2: // Recieved payment & success
                            $dvalue['status_message']= '<span class="label label-warning">Draft</span>&nbsp;'.$extra;
                            break;
                        case 3: // Recieved with error
                            $dvalue['status_message']= '<span class="label label-danger">Deleted</span>&nbsp;'.$extra;
                            break;

                        default:
                            $dvalue['status_message']= '<span class="label label-default">Deleted</span>&nbsp;'.$extra;
                            break;
